...
tf.app.flags.DEFINE_float('conf_threshold', 0.05, 'Confidence threshold')
...
def print_boxes(boxes, img, cls_names, detection_size):
    ...
    for cls in [CAR_CLASS]:
        for box, score in boxes[0][cls]:
            ...
            # box = convert_to_original_size(box, np.array(detection_size), np.array(img.size))
            # print("class number {} is {} has score {:.2f} \% and box {}".format(cls,cls_names[cls],score * 100,box))
            # draw.rectangle(box, outline=color)
            # draw.text(box[:2], '{} {:.2f}%'.format(cls_names[cls], score * 100), fill=color)
