...
tf.app.flags.DEFINE_integer('z_dim', 2, 'the size of the z latent vector ')
tf.app.flags.DEFINE_integer('network_size', 3, 'the size of hidden layers of the G and D networks')
...
tf.app.flags.DEFINE_float('learning_rate_t',0.0001, 'the adam learning rate of the D')
tf.app.flags.DEFINE_float('learning_rate_g',0.0001, 'the adam learning rate of the G ')
...
def randomize_setup(flags):
    ...
    induced_size_list = list(range(200,500))
    nb_steps_list = list(range(200,800))
    variamce_list = list(np.linspace(0.05,0.07,20))
    learning_rate_list  = list(np.linspace(0.00005,0.0005,20))
